...
class ActorNetwork(tf.keras.Model):
    def __init__(self, input_dims, n_actions, fc1_dims=512, fc2_dims=512, learning_rate=1e-4, weight_decay=1e-2):
...
class Agent:
    def __init__(self, state_dims, action_dims, action_max, mem_size, gamma=0.99, tau=0.001, batch_size=64):
        ...
        self.action_max = tf.convert_to_tensor(action_max, dtype=tf.float32)
    ...
    def get_action(self, state, training=True):
        ...
        if training:
            action += tf.random.normal(shape=action.shape, mean=0.0, stddev=0.1)
            action = tf.clip_by_value(action, clip_value_min=-1.0, clip_value_max=1.0)
            action = tf.multiply(action, self.action_max)
        else:
            action = tf.multiply(action, self.action_max)
