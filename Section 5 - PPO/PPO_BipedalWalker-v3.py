...
class ValueNetwork(nn.Module):
    def __init__(self, state_dims, fc1_dims=400, fc2_dims=300, learning_rate=3e-4):
...
class ActorNetwork(nn.Module):
    def __init__(self, state_dims, n_actions, fc1_dims=400, fc2_dims=300, learning_rate=1e-4):
...
def main(training=True):
    ...
    if training:
        ...
        axes = plt.axes()
        axes.set_ylim([np.min(reward_set) - 100, np.max(reward_set) + 50])
        plt.xlabel('Episode')
        ...
        plt.show()
        plt.savefig(dir_ + '/Rewards_BipedalWalker-v3')
