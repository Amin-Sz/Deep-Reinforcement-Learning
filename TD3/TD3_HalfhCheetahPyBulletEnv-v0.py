...
class ActorNetwork(nn.Module):
    def __init__(self, state_dim, action_dim, action_max, fc1_dims=400, fc2_dims=300, learning_rate=1.5e-3):
...
def main(training=True):
    ...
    if training:
        ...
        axes = plt.axes()
        axes.set_ylim([np.min(reward_set) - 100, np.max(reward_set) + 350])
        plt.xlabel('Episode')
        plt.ylabel('Reward')
        ...
        plt.show()
        plt.savefig(dir_ + '/Rewards_HalfCheetahPyBulletEnv')
