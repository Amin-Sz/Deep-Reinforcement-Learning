...
class ActorNetwork(tf.keras.Model):
    def __init__(self, state_dims, action_dims, fc1_dims=256, fc2_dims=256, learning_rate=5e-6):
...
def main(training):
    ...
    solved_score = 200
    dir_ = 'Section 1 - Actor-Critic/LunarLanderContinuous-v2'
    gamma = 0.99
    ...
    if training:
        ...
        axes = plt.axes()
        axes.set_ylim([np.min(reward_set) - 150, np.max(reward_set) + 10])
        plt.xlabel('Episode')
        ...
        plt.show()
        plt.savefig(dir_ + '/Rewards_LunarLanderContinuous-v2')
